...
func (r *Repo) Read(ctx context.Context, filter entity.UserFilter) ([]entity.User, error) {
	log.Println("user read started")
	defer log.Println("user read done")
	...
	if len(filter.Interests) > 0 {
		q = q.Where(squirrel.Eq{"interests": filter.Interests})
	}
...
func (r *Repo) Delete(ctx context.Context, filter entity.UserFilter) error {
	...
	defer log.Println("user delete done")
	q := r.builder.Delete(entity.TableUser)
	...
	if err != nil {
		log.Printf("unable to delete users: %v\n", err)
