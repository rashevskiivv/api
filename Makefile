...
deploy-pg:
	cd deployment && docker-compose ${DC_FILE} -p ${RELEASE_NAME} up -d api_db postgres_migrate
...
delete-pg:
	cd deployment && docker-compose ${DC_FILE} -p ${RELEASE_NAME} rm -sf api_db postgres_migrate
